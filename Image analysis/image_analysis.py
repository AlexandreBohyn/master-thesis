...
def area_percentage(img_path,thresh_val,flipBool,BoundingArray):
   ...
   """
    ...
    if(flipBool):
        img = cv2.flip(img, 1 );
    img = img[BoundingArray[0]:BoundingArray[0]+BoundingArray[2],BoundingArray[1]:BoundingArray[1]+BoundingArray[3]];
...
for i in range(data_filtered.size):
    ...
    folder = '2019-' + data_filtered.Folder[i][3:5] + '-' + data_filtered.Folder[i][:2];
