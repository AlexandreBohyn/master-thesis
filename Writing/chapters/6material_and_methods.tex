...
As explained in the literature review chapter, the standard spatial models (SS models), such as the $AR\timesAR$ model, use a spatially dependant error term to adjust for the local trends, whereas the SpATS model encompasses them in the smooth surface. To estimate the spatial covariance structure of this error process, \textcite{gilmour_accounting_1997} advocate the use of variograms and more precisely, the use of auto-regressive processes ($AR(1)$) to model those variograms. The general representation of the SS model is very to the SpATS model:
\begin{equation}
	y = \mathbf{X}\boldsymbol{\beta} + \mathbf{X}_{s}\boldsymbol{\beta}_{s} + \mathbf{Zc} +  \mathbf{Z}_{s}\mathbf{c}_{s} + 
	\boldsymbol{\xi} + \bolsymbol{\varepsilon}
	\text{,}
	\label{eq:standard_spatial_model}
\end{equation}
where $\mathbf{X}\boldsymbol{\beta}$ contains the fixed terms of the model, $\mathbf{Zc}$ contains the random terms, $\bolsymbol{\varepsilon}$ is the vector of the residuals and $\boldsymbol{\xi}$ is the vector of spatially dependant residuals modelling the local trend. However, unlike the SpATS model, $\mathbf{X}_{s}\boldsymbol{\beta}_{s}$ and $\mathbf{Z}_{s}\mathbf{c}_{s}$ do not model a smooth bivariate surface, but contain linear and one-dimensional cubic splines terms that account for the global variations in the data (see \textcite{verbyla_analysis_1999} for details).\\

\subsection{Variogram and $AR(1)$ process}
The term of interest in equation \ref{eq:standard_spatial_model} is the spatially dependant error term $\boldsymbol{\xi}$.
talk aout:  variogram and how an AR process is used to  model it

\subsection{Linear variance structure}
brifly how the LV component can be used to model it (+ representation from sas https://documentation.sas.com/?docsetId=statug&docsetVersion=14.3&docsetTarget=statug_mixed_syntax14.htm&locale=en)
